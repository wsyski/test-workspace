...
export interface EnabledDatesClayDatePickerProps extends IProps {
	enabledDays?: (day: Date) => boolean;
}

/**
 * ClayDatePicker component.
 */
const EnabledDatesClayDatePicker = React.forwardRef<HTMLInputElement, EnabledDatesClayDatePickerProps>(
	(
		{
			ariaLabels = {
			...
			yearsCheck = true,
			...otherProps
		}: EnabledDatesClayDatePickerProps,
		ref
	) => {
									...
									<DaysTable weeks={weeks}>
										{({day, key}) => (
											<DayNumber
												...
												disabled={disabled || !enabledDays(day.date)}
												index={key}
...
export default EnabledDatesClayDatePicker;
