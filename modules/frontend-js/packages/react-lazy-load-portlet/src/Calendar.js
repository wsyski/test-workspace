...
class Calendar extends Component {
  ...
  handleNext = event => {
    event && event.preventDefault();
    const fn = event.shiftKey ? this.gotoNextYear : this.gotoNextMonth;
    this.handlePressure(fn);
  };
  ...
  renderDayLabel = (day, index) => {
    ...
    return (
      <Styled.CalendarDay index={index} key={daylabel}>
        {daylabel}
      </Styled.CalendarDay>
    );
  };
  ...
  renderCalendarDate = (date, index) => {
    const { current, month, today, year } = this.state;
    ...
    const props = { inMonth, index, onClick, title: _date.toDateString() };
